...
execute if score @s nations_dowsing_cooldown matches 1.. run scoreboard players operation #TEMP nations_pid = @s nations_dowsing_cooldown
execute if score @s nations_dowsing_cooldown matches 1.. run scoreboard players set #CONST_20 nations_pid 20
execute if score @s nations_dowsing_cooldown matches 1.. run scoreboard players operation #TEMP nations_pid /= #CONST_20 nations_pid
execute if score @s nations_dowsing_cooldown matches 1.. run title @s actionbar [{"text":"Dowsing Rod on cooldown: ","color":"red"},{"score":{"name":"#TEMP","objective":"nations_pid"},"color":"gold"},{"text":"s","color":"red"}]
